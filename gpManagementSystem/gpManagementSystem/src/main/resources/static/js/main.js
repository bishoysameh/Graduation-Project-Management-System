...
//     //make this the sender username instead user wite his name
...
// //the message you sent is stored in database can you get all chatRoom messages when restart server by calling chatRoomMessages Api 
...
function onConnected() {
    // Subscribe to chat room messages (replace 1 with dynamic room ID if needed)
    stompClient.subscribe('/chatroom/1', onMessageReceived);
    connectingElement.classList.add('hidden');
}
...
function sendMessage(event) {
    ...
    if (messageContent && stompClient) {
        const chatMessage = {
            chatRoomId: 1,           // Replace with actual room ID
            senderId: 1554,          // Replace with actual user ID
            sender: username,        // Replace with actual username if needed
